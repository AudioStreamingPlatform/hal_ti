...
let config = [
    {
        ...
        onChange: (inst, ui) => {
            ...
            // input specifics
            ui.hysteresisControl.hidden = hideInput || hideAll;
            ui.wakeupLogic.hidden = hideInput || hideAll;
            // output specifics
            ui.driveStrength.hidden = !hideInput || hideAll;
            ui.hiZ.hidden = !hideInput || hideAll;

            /* Special Case: disabling output inversion */
            ui.invert.hidden = (hideInput && inst.hideOutputInversion) || hideAll;
        }
    },
    {
        name: "pinIOStructure",
...
function validatePinmux(inst,validation){
    ...
    let pinIOStructure = system.deviceData.gpio.pinInfo[inst.$ownedBy.peripheral[inst.passedResourceName].$solution.packagePinName].devicePin.attributes.io_type;
